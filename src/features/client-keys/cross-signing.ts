...
import { generateOpaqueId, toUserId } from "../../shared/utils/ids";
import { runClientEffect } from "../../matrix/application/runtime/effect-runtime";
import { loadGlobalAccountDataEffect } from "../account-data/storage";
import {
...
export async function uploadCrossSigningKeys(input: {
  ...
  await storeCrossSigningKeysToDO(env as AppEnv["Bindings"], userId, csKeys);
  await storeCrossSigningKeysBackup(env.DB, userId, csKeys, Boolean(master_key));
  await cacheCrossSigningKeys(env as AppEnv["Bindings"], userId, csKeys);
      ...
      try {
        ...
        if (signedKeyObj.device_id) {
          ...
          if (existingKey) {
            ...
            await storeDeviceKeysToDO(env as AppEnv["Bindings"], userId, deviceId, existingKey);
            await cacheDeviceKeys(env as AppEnv["Bindings"], userId, deviceId, existingKey);
          } else {
            await runClientEffect(
