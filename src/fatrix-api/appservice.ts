...
import { getAppServiceByToken } from "../platform/cloudflare/adapters/integrations/appservice";
...
app.get("/_matrix/app/v1/rooms/:roomAlias", requireAppServiceAuth, async (c) => {
  ...
  const result = await c.env.DB.prepare(`SELECT room_id FROM room_aliases WHERE alias = ?`)
    .bind(roomAlias)
