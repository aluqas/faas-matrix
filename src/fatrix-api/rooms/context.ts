...
async function waitForPartialStateJoinCompletion(
  ...
  while (Date.now() < deadline) {
    ...
    await new Promise<void>((resolve) => {
      setTimeout(resolve, completion ? 25 : 100);
    });
  }
}
...
app.get("/_matrix/client/v3/rooms/:roomId/context/:eventId", requireAuth(), async (c) => {
  ...
  return c.json({
    ...
    events_before: eventsBefore.results.toReversed().map((event) => formatContextEvent(event)),
    events_after: eventsAfter.results.map((event) => formatContextEvent(event)),
