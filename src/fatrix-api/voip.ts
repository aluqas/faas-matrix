...
import {
  ...
  isTurnConfigured,
  TurnError,
} from "../platform/cloudflare/adapters/integrations/turn";
import { notifyUsersOfEvent } from "../platform/cloudflare/adapters/db/database";
...
app.put("/_matrix/client/v1/rooms/:roomId/call", requireAuth(), async (c) => {
  ...
  `)
    .bind(roomId, userId, eventId, JSON.stringify(content), userId, now)
    .run();
  ...
  await db
    ...
    VALUES (?, ?, 'm.call.member', ?, ?, ?, ?)
  `)
    .bind(eventId, roomId, userId, JSON.stringify(content), userId, now)
    .run();
...
app.delete("/_matrix/client/v1/rooms/:roomId/call", requireAuth(), async (c) => {
  ...
  await db
    ...
    WHERE room_id = ? AND type = 'm.call.member' AND state_key = ?
  `)
    .bind(eventId, JSON.stringify(content), userId, now, roomId, userId)
    .run();
