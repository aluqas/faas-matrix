...
import type { EventId, RoomId, UserId } from "../fatrix-model/types";
...
import { countNotificationsWithRules } from "../platform/cloudflare/adapters/realtime/push-rule-evaluator";
import type { ConnectionState } from "../fatrix-backend/application/features/sync/ports/effect-ports";
import {
...
} from "../fatrix-backend/application/features/sync/projectors/sliding-sync-shared";
import { buildSlidingSyncVisibilityContext } from "../fatrix-backend/application/features/sync/types/contracts";
import {
  getEffectiveJoinedMemberCount,
  getJoinedRoomIdsIncludingPartialState,
} from "../platform/cloudflare/adapters/repositories/membership-repository";
import {
...
> {
  ...
  for (const row of result.results as any[]) {
    ...
    if (
      filters?.room_name_like &&
      name &&
      !name.toLowerCase().includes(filters.room_name_like.toLowerCase())
    ) {
      continue;
    }
