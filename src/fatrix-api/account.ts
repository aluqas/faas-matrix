...
import { hashPassword, hashToken, verifyPassword } from "../fatrix-model/utils/crypto";
...
import {
  ...
  validateEmailToken,
  getValidatedSession,
} from "../platform/cloudflare/adapters/integrations/email";
...
app.post("/_matrix/client/v3/account/3pid/add", requireAuth(), async (c) => {
  ...
  await db
    ...
    VALUES (?, 'email', ?, ?, ?)
  `)
    .bind(userId, validatedSession.email, now, now)
    .run();
...
app.post("/_matrix/client/v3/account/3pid/email/requestToken", async (c) => {
  ...
  if (result.token) {
    ...
    const emailResult = await sendVerificationEmail(c.env, email, result.token, c.env.SERVER_NAME);
...
app.post("/_matrix/client/v3/user/:userId/openid/request_token", requireAuth(), async (c) => {
  ...
  await c.env.CACHE.put(`openid_token:${accessToken}`, JSON.stringify(tokenData), {
