...
import type { AuthorizationCode, OAuthClient, OAuthToken } from "../fatrix-model/types/client";
import { requireAuth } from "./middleware/auth";
import { hashToken, verifyPassword } from "../fatrix-model/utils/crypto";
import {
  generateAccessToken,
  generateDeviceId,
  generateOpaqueId,
...
app.post("/oauth/authorize", async (c) => {
  ...
  if (!username || !password || !authRequestId) {
    return c.html(
      generateLoginPage("", authRequestId || "", c.env.SERVER_NAME, "Missing username or password"),
    );
  }
  ...
  if (!user || !user.password_hash) {
    ...
    await c.env.SESSIONS.put(
      `oauth_auth_request:${newAuthRequestId}`,
...
app.post("/oauth/token", async (c) => {
  ...
  if (grantType === "authorization_code") {
    ...
    await createDevice(
      ...
      authCode.user_id,
      deviceId,
      `OAuth Client (${client.client_name})`,
    );
...
app.post("/oauth/revoke", async (c) => {
  ...
  if (!tokenTypeHint || tokenTypeHint === "access_token") {
    ...
    await c.env.DB.prepare("DELETE FROM access_tokens WHERE token_hash = ?").bind(tokenHash).run();
  }
...
app.post("/oauth/introspect", async (c) => {
  ...
  const accessToken = await c.env.DB.prepare(
    ...
    .bind(tokenHash)
    .first<{ user_id: string; device_id: string; created_at: number }>();
...
app.get("/oauth/authorize/uia", async (c) => {
  ...
  return c.html(
    generateUiaApprovalPage(sessionId, session.user_id, actionTitle, actionDescription, serverName),
  );
});
