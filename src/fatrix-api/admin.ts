...
import { isJsonObject, type JsonObject } from "../fatrix-model/types/common";
import { Errors } from "../fatrix-model/utils/errors";
import { requireAuth } from "./middleware/auth";
import { getUserById } from "../platform/cloudflare/adapters/db/database";
import { generateLoginToken, generateOpaqueId, toUserId } from "../fatrix-model/utils/ids";
import { hashToken } from "../fatrix-model/utils/crypto";
import { encryptSecret } from "./oidc-auth";
import { fetchOIDCDiscovery } from "../platform/cloudflare/adapters/integrations/oidc";
...
function getAdminDO(env: import("../platform/cloudflare/env").Env) {
  const id = env.ADMIN.idFromName("global");
  return env.ADMIN.get(id);
}

// Invalidate stats cache after data-modifying operations
async function invalidateStatsCache(env: import("../platform/cloudflare/env").Env) {
...
app.delete("/admin/api/users/:userId", requireAuth(), requireAdmin, async (c) => {
  ...
  await db
    .prepare("UPDATE users SET is_deactivated = 1, updated_at = ? WHERE user_id = ?")
...
app.post("/admin/api/users/:userId/reset-password", requireAuth(), requireAdmin, async (c) => {
  ...
  const { hashPassword } = await import("../fatrix-model/utils/crypto");
...
app.get("/admin/api/rooms", requireAuth(), requireAdmin, async (c) => {
  ...
  const roomsWithNames = await Promise.all(
    rooms.results.map(async (room: any) => {
  ...
  try {
    ...
    if (resp.ok) {
      ...
      tests.push({
        ...
        message: hasSigningKeys
          ? `${Object.keys(data.verify_keys).length} key(s) published`
...
app.delete("/admin/api/media/:mediaId", requireAuth(), requireAdmin, async (c) => {
  ...
  const thumbnails = await db
    .prepare("SELECT width, height, method FROM thumbnails WHERE media_id = ?")
    .bind(mediaId)
    .all();
...
app.post("/admin/api/make-admin", requireAuth(), requireAdmin, async (c) => {
  ...
  await c.env.DB.prepare("UPDATE users SET admin = 1, updated_at = ? WHERE user_id = ?")
...
app.post("/admin/api/remove-admin", requireAuth(), requireAdmin, async (c) => {
  ...
  await c.env.DB.prepare("UPDATE users SET admin = 0, updated_at = ? WHERE user_id = ?")
    .bind(Date.now(), user_id)
    .run();
...
app.delete("/admin/api/users/:userId/purge", requireAuth(), requireAdmin, async (c) => {
  ...
  const media = await db
    .prepare("SELECT media_id FROM media WHERE user_id = ?")
    .bind(userId)
    .all<{ media_id: string }>();
  for (const m of media.results) {
    ...
    const thumbs = await db
      .prepare("SELECT width, height, method FROM thumbnails WHERE media_id = ?")
      .bind(m.media_id)
      .all<{ width: number; height: number; method: string }>();
  ...
  await db
    .prepare(
      "DELETE FROM thumbnails WHERE media_id IN (SELECT media_id FROM media WHERE user_id = ?)",
    )
    .bind(userId)
    .run();
  ...
  // Messaging
  await db
    .prepare("DELETE FROM to_device_messages WHERE recipient_user_id = ? OR sender_user_id = ?")
    .bind(userId, userId)
    .run();
...
app.post("/admin/api/users/:userId/reactivate", requireAuth(), requireAdmin, async (c) => {
  ...
  await db
    .prepare("UPDATE users SET is_deactivated = 0, updated_at = ? WHERE user_id = ?")
    .bind(Date.now(), userId)
    .run();
...
app.delete("/admin/api/users/:userId/sessions", requireAuth(), requireAdmin, async (c) => {
  ...
  const result = await db.prepare("DELETE FROM access_tokens WHERE user_id = ?").bind(userId).run();
...
app.delete("/admin/api/sessions/:sessionId", requireAuth(), requireAdmin, async (c) => {
  ...
  await db.prepare("DELETE FROM access_tokens WHERE token_id = ?").bind(sessionId).run();
...
app.post("/admin/api/server-notice", requireAuth(), requireAdmin, async (c) => {
  ...
  const messageId = `notice_${Date.now()}_${Math.random().toString(36).slice(2, 11)}`;
...
app.get("/admin/api/users/:userId/keys", requireAuth(), requireAdmin, async (c) => {
  ...
  const crossSigningKeys = await db
    .prepare(`
    SELECT key_type, key_id, key_data FROM cross_signing_keys WHERE user_id = ?
  `)
    .bind(userId)
    .all<{ key_type: string; key_id: string; key_data: string }>();
  ...
  `)
    .bind(userId)
    .all<{ key_id: string; signer_user_id: string; signer_key_id: string; signature: string }>();
...
app.get("/_synapse/admin/v1/rooms", requireAuth(), requireAdmin, async (c) => {
  ...
  const roomsWithDetails = await Promise.all(
    ...
    `)
      ...
      const canonicalAliasEvent = await db
        .prepare(`
      SELECT e.content FROM room_state rs
      JOIN events e ON rs.event_id = e.event_id
...
app.get("/_synapse/admin/v1/event_reports", requireAuth(), requireAdmin, async (c) => {
  ...
  const total = await db
    .prepare("SELECT COUNT(*) as count FROM content_reports")
    .first<{ count: number }>();
