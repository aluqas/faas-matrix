...
import type { KeysUploadRequest } from "../../../../fatrix-model/types/client";
...
import { queueFederationEdu } from "../../../../platform/cloudflare/adapters/application-ports/shared/federation-edu-queue";
      ...
      await publishDeviceListUpdateToSharedServers(
        ...
        },
        {
          ...
          queueEdu: (destination, eduType, content) =>
            queueFederationEdu(env as Env, destination, eduType, content),
        },
      );
    } catch (fedErr) {
      await runClientEffect(
