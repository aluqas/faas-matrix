...
import { shouldIncludeRoom } from "../../../orchestrators/sync-projection";
import type { SyncRepository } from "../../../../ports/repositories";
import type { PartialStatePort, SyncQueryPort } from "../ports/effect-ports";
import { buildSyncToken, parseSyncToken, type SyncAssemblerInput } from "../types/contracts";
...
import { projectTopLevelSync, type TopLevelSyncPorts } from "./top-level";
...
export function assembleSyncResponseEffect(
  ...
  return Effect.gen(function* () {
    ...
    const topLevel = yield* Effect.tryPromise({
      try: () =>
        projectTopLevelSync(
          { repository: ports.repository, ...ports.topLevel },
          {
            userId: input.userId,
