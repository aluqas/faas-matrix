...
import type { FilterDefinition, SyncRepository } from "../../ports/repositories";
import { toEventId } from "../../../fatrix-model/utils/ids";
...
export async function projectMembershipRooms(
  ...
  for (const roomId of invitedRoomIds) {
    ...
    const inviteEventId = membership.eventId ? toEventId(membership.eventId) : null;
    const inviteEvent =
      (inviteEventId ? await repository.getEvent(inviteEventId) : null) ??
      roomState.find(
        (event) =>
          event.type === "m.room.member" &&
  ...
  for (const roomId of knockedRoomIds) {
    ...
    const knockEventId = toEventId(membership.eventId);
    const knockEvent = knockEventId ? await repository.getEvent(knockEventId) : null;
    if (
      knockEvent &&
  ...
  for (const roomId of leaveLikeRoomIds) {
    ...
    if (!leaveEvent) {
      ...
      if (membership?.membership === "leave" || membership?.membership === "ban") {
        const leaveEventId = toEventId(membership.eventId);
        const event = leaveEventId ? await repository.getEvent(leaveEventId) : null;
