...
} from "../../platform/cloudflare/adapters/repositories/room-query-repository";
...
import { getPartialStateDeferredAuthReason } from "./federation/transactions/partial-state-membership";
...
export class MatrixRoomQueryService {
  ...
  getCurrentState(
    ...
    return Effect.gen(function* () {
      ...
      return state.map((event) => toRoomEventResponse(event));
    });
  }
    ...
    return Effect.gen(function* () {
      ...
      return {
        chunk: events
          ...
          .map((event) => toRoomEventResponse(event)),
      };
    });
  }
  ...
  > {
    ...
    return Effect.gen(function* () {
      ...
      return {
        ...
        chunk: events.map((event) => toRoomEventResponse(event)),
      };
    });
  }
